...
MAKE_OPT="make -j $(nproc) TARGET=linux-glibc ARCH=x86_64 USE_OPENSSL=1 USE_LUA=1 USE_PCRE=1 USE_SYSTEMD=1 USE_ZLIB=1 USE_SYSTEMD=1 USE_CPU_AFFINITY=1 PREFIX=$basepath/apps/$app_dir"
...
_make_install() {
    ...
    mkdir apps/$app_dir
    cd ${app_name}-${version}
    sudo $MAKE_OPT && sudo make install
